...
function ImprimirIvaVentas(Data,header,totales,Encab) {
    ...
    doc.text("Reporte de Iva Ventas", xText, 25);
   ...
   // const tableData = data.map(row => Object.values(row));
    ...
    const columnStyles = {
        0: { columnWidth: 25 }, // Ancho de la primera columna
        1: { columnWidth: 10 }, // Ancho de la segunda columna
        2: { columnWidth: 10 }, // Ancho de la segunda columna
        3: { columnWidth: 30 }, // Ancho de la segunda columna
        4: { columnWidth: 50 }, // Ancho de la segunda columna
        5: { columnWidth: 50 }, // Ancho de la segunda columna
        6: { columnWidth: 145 }, // Ancho de la segunda columna
        7: { columnWidth: 50 }, // Ancho de la segunda columna
        8: { columnWidth: 50 }, // Ancho de la segunda columna
        9: { columnWidth: 50 }, // Ancho de la segunda columna
       10: { columnWidth: 50 }, // Ancho de la segunda columna

    };

       // Crear un array de arrays para los datos de la tabla
    const tableData = data.map((rowData) => {
        return Object.values(rowData).map((value, index) => {
            if (typeof value === 'string') {
                if (index < 7) {
                    // Verificar si value es una cadena de texto y el índice es mayor a 6
                    return value.replace(/"/g, ''); // Eliminar comillas de los strings
                } else {
...
/*
    // Configuración de las columnas para redondear a 2 decimales
    const columns = [
        'neto',
        'gastos',
        'totalIva',
    ...
    let newData = tableData.map(rowData => {
        return rowData.map((value, index) => {
            // Verificar si la columna actual debe redondearse
...
*/
    ...
    doc.autoTable({
        ...
        willDrawPage: function (data) {
            ...
            doc.setFontSize(20)
            doc.setTextColor(40)
            doc.text("Reporte de Iva Ventas ", data.settings.margin.left + 15, 25, { align: "center" });
    ...
    var Reporte = 'Reporte_Iva_Ventas.pdf'
    doc.save(Reporte);
}
